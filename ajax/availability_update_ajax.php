...
<?php
  ...
  //   }
      ...
      $close_av = "UPDATE availability SET close_date=('$date_calculate_end') WHERE not id_class in ($id)";
      $close_result = dbQuery($close_av);

      //delete selected before classes from the list  (v -> v)
      $checked_lines = dbQuery("SELECT id_class FROM availability WHERE id_class in ($id)");
